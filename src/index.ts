...
server.tool(
	'create-contact',
	'Create a new contact (Neukunde/Lieferant) in Lexware Office. Provide companyName for a company, or firstName/lastName for a person. After creation the contact is re-fetched so the assigned customer/vendor number is included in the response.',
	{
		...
		taxNumber: z.string().optional().describe('Tax number (Steuernummer) of the company, e.g. "123/456/78901"'),
		vatRegistrationId: z.string().optional().describe('VAT registration ID (Umsatzsteuer-ID) of the company, e.g. "DE123456789"'),
		firstName: z.string().optional(),
		lastName: z.string().optional().describe('Required if companyName is not provided'),
		salutation: z.string().optional(),
		billingStreet: z.string().optional().describe('Billing address: street and house number, e.g. "Musterstraße 12"'),
		billingZip: z.string().optional().describe('Billing address: postal code, e.g. "10115"'),
		billingCity: z.string().optional().describe('Billing address: city, e.g. "Berlin"'),
		billingCountryCode: z.string().length(2).optional().describe('Billing address: ISO 3166-1 alpha-2 country code, e.g. "DE"'),
		billingAddressSupplement: z.string().optional().describe('Billing address: optional supplement / additional address line'),
		emailBusiness: z.string().email().optional().describe('Business email address (Geschäftliche E-Mail)'),
		note: z.string().optional(),
	},
	async ({ customer, vendor, companyName, taxNumber, vatRegistrationId, firstName, lastName, salutation,
		billingStreet, billingZip, billingCity, billingCountryCode, billingAddressSupplement, emailBusiness, note }) => {
		if (!customer && !vendor) {
		...
		const billingEntry = hasBillingAddress ? {
			street: billingStreet,
			zip: billingZip,
			city: billingCity,
			countryCode: billingCountryCode ?? 'DE',
			...(billingAddressSupplement ? { supplement: billingAddressSupplement } : {}),
		} : undefined;
		...
		const contactId = (result.data as any)?.id ?? 'unknown';
		auditAllow('create-contact', { id: contactId });
		// Re-fetch to include the assigned customer/vendor number
		const details = await makeLexwareOfficeRequest<any>(`/v1/contacts/${contactId}`);
		const contactData = details.ok ? details.data : result.data;
		const customerNumber = contactData?.roles?.customer?.number ?? null;
		const vendorNumber = contactData?.roles?.vendor?.number ?? null;
		const numberInfo = [
			customerNumber !== null ? `Kundennummer: ${customerNumber}` : null,
			vendorNumber !== null ? `Lieferantennummer: ${vendorNumber}` : null,
		].filter(Boolean).join(', ');
		return { content: [{ type: 'text', text: `Contact created successfully${numberInfo ? ` (${numberInfo})` : ''}:\n\n${JSON.stringify(contactData, null, 2)}` }] };
	},
);
...
server.tool(
	...
	'Update an existing contact in Lexware Office. Requires the current version number for optimistic locking (get it from get-contact-details).',
	{
		...
		taxNumber: z.string().optional().describe('Tax number (Steuernummer), e.g. "123/456/78901"'),
		vatRegistrationId: z.string().optional().describe('VAT registration ID (Umsatzsteuer-ID), e.g. "DE123456789"'),
		firstName: z.string().optional(),
		lastName: z.string().optional(),
		salutation: z.string().optional(),
		billingStreet: z.string().optional().describe('Billing address: street and house number'),
		billingZip: z.string().optional().describe('Billing address: postal code'),
	...
	async ({ id, version, customer, vendor, companyName, taxNumber, vatRegistrationId, firstName, lastName, salutation,
		billingStreet, billingZip, billingCity, billingCountryCode, billingAddressSupplement, emailBusiness, note }) => {
		if (!customer && !vendor) {
